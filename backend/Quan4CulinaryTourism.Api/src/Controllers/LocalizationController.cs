...
public class LocalizationController : BaseApiController
{
    ...
    public Task<IActionResult> GetLocalizations(string id) => ExecuteAsync(() => _service.GetPoiLocalizationsAsync(id), "Lấy localizations thành công");

    [HttpPost($"{AppConstants.ApiVersionPrefix}/admin/pois/{{id}}/localizations")]
    public Task<IActionResult> Create(string id, [FromBody] CreatePoiLocalizationRequest request) => ExecuteAsync(() => _service.CreateAsync(id, request), "Tạo localization thành công");

    [HttpPut($"{AppConstants.ApiVersionPrefix}/admin/pois/{{id}}/localizations/{{lang}}")]
    public Task<IActionResult> Update(string id, string lang, [FromBody] UpdatePoiLocalizationRequest request) => ExecuteAsync(() => _service.UpdateAsync(id, lang, request), "Cập nhật localization thành công");

    [HttpDelete($"{AppConstants.ApiVersionPrefix}/admin/pois/{{id}}/localizations/{{lang}}")]
    public Task<IActionResult> Delete(string id, string lang) => ExecuteAsync(() => _service.DeleteAsync(id, lang), "Xóa localization thành công");
}
